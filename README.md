...
Home Assistant gets three buttons: **Animate Arm** (arm sweep only), **Meow** (random meow only), and **Meow and Arm** (both). These manual actions ignore mute and the meow rate limit; the arm sweep is still skipped if the arm is already moving.
